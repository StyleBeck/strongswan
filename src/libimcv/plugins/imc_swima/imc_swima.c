							...
							swima_inventory_t *targets)
{
	...
	if (targets->get_eid(targets, NULL) > 0)
	{
		...
		if (!sw_ev)
		{
			snprintf(error_msg, sizeof(error_msg), "failed to collect software"
					 "%s events", sw_id_only ? " ID" : "");
			attr = swima_error_create(PA_ERROR_SW, request_id, 0, error_msg);
		}
		else {
			items = sw_ev->get_count(sw_ev);
			DBG1(DBG_IMC, "collected %d SW%s event%s", items ,
				 sw_id_only ? " ID" : "", items == 1 ? "" : "s");
	...
	else
	{
		...
		{
			snprintf(error_msg, sizeof(error_msg), "failed to collect software"
					 "%s inventory", sw_id_only ? " ID" : "");
			attr = swima_error_create(PA_ERROR_SW, request_id, 0, error_msg);
		}
		else
		{
			items = sw_inv->get_count(sw_inv);
			DBG1(DBG_IMC, "collected %d SW%s record%s", items ,
				 sw_id_only ? " ID" : "", items == 1 ? "" : "s");
