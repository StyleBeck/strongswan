    ...
    private_imv_swima_state_t *this, swima_events_t *events)
{
	...
	while (enumerator->enumerate(enumerator, &sw_event))
	{
		...
		json_object_object_add(this->jobj, "eid", jvalue);
		jstring = json_object_new_string(timestamp_str);
		json_object_object_add(this->jobj, "timestamp", jstring);
		jvalue = json_object_new_int(record_id);
		json_object_object_add(this->jobj, "recordId", jvalue);
		jvalue = json_object_new_int(source_id);
		json_object_object_add(this->jobj, "sourceId", jvalue);
		jvalue = json_object_new_int(action);
		json_object_object_add(this->jobj, "action", jvalue);
		jstring = json_object_new_string(sw_id_str);
		json_object_object_add(this->jobj, "swIdentifier", jstring);
